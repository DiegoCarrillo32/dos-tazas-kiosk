  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-expresso">Analytics</h1>
          <p className="text-expresso/60 mt-1">Store performance and sales insights</p>
        </div>
        <div className="flex items-center gap-3">
          <DatePicker date={startDate} setDate={setStartDate} placeholder="Start Date" className="w-40" />
          <span className="text-expresso/40">to</span>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div className="bg-card p-6 rounded-2xl border border-warm-roast/10 shadow-sm">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-sm font-medium text-expresso/60">Total Revenue</p>
            ...
            <div className="bg-card p-6 rounded-2xl border border-warm-roast/10 shadow-sm">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-sm font-medium text-expresso/60">Total Orders</p>
            ...
            <div className="bg-card p-6 rounded-2xl border border-warm-roast/10 shadow-sm">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-sm font-medium text-expresso/60">Avg Order Value</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Revenue Chart */}
            <div className="bg-card p-6 rounded-2xl border border-warm-roast/10 shadow-sm">
              <h3 className="text-lg font-bold text-expresso mb-6">Revenue Trend</h3>
            ...
            {/* Orders by Hour */}
            <div className="bg-card p-6 rounded-2xl border border-warm-roast/10 shadow-sm">
              <h3 className="text-lg font-bold text-expresso mb-6">Productive Hours</h3>
              <div className="h-[300px] w-full">
                <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="bg-card rounded-2xl border border-warm-roast/10 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-warm-roast/10">
              <h3 className="text-lg font-bold text-expresso">Top Selling Items</h3>
            </div>
                ...
                <tbody className="divide-y divide-warm-roast/10">
                  {data.topItems.map((item, i) => (
                    <tr key={i} className="hover:bg-warm-roast/5 transition-colors">
                      ...
                      <td className="px-6 py-4 text-expresso/60">{item.quantity} units</td>
                      <td className="px-6 py-4 text-expresso/60">${item.revenue.toFixed(2)}</td>
                    </tr>
                  ))}
                  {data.topItems.length === 0 && (
                    <tr>
                      <td colSpan={3} className="px-6 py-8 text-center text-expresso/60">No items sold in this period.</td>
                    </tr>
                  )}
                </tbody>
              </table>
